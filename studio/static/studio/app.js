...
cards.forEach((card, i) => {
  card.querySelector('.plot-title').textContent =
    plots[i][0].toUpperCase();

  card.querySelector('.plot-description').textContent =
    plots[i][1];
...
function enqueue(
  ...
  const run = async () => {
    ...
    try {
      ...
      notice(
        error.message ||
        'Connection lost. Check that the Django server is running.',
        true
      );
...
async function mutate(
  ...
  try {
    ...
    if (
      ...
      fallback =
        'Reference target selected. Open a video to begin global searching.';
      ...
      fallback =
        'Reference target ready. Press Play: global search will continue until the object is found.';
      ...
      fallback =
        'Target initialized. Press Play to follow the signal.';

    } else {

      fallback =
        'Select a target in the video, or upload a target image and select its target region.';
    }
...
async function tick(epoch) {
  ...
  try {
    ...
    if (s.ended) {
      ...
      notice(
        'End of video. Export your session or restart the source.'
      );
    ...
    displayedTimes =
      displayedTimes.slice(-30);

    if (
      displayedTimes.length > 1
    ) {
      ...
      $('frame-label').textContent =
        `FRAME ${s.index + 1} · ${actual.toFixed(1)} FPS ACTUAL`;
    }
...
function togglePlay() {
  ...
  if (playing) {
    ...
    notice(
      'Paused. Inspect the current frame, or step forward.'
    );
...
canvas.addEventListener(
  ...
  async event => {
    ...
    await mutate(
      'select',
      {bbox},
      'Video target initialized. Press Play or step one frame.'
    );
  }
);
...
function setReferenceImage(file) {
  ...
  img.onerror =
    () => {
      ...
      notice(
        'Could not preview this target image.',
        true
      );
    };
...
function drawReference() {
  ...
  if (selected) {
    ...
    referenceCtx.strokeStyle =
      '#6ee7be';

    referenceCtx.lineWidth =
      2 *
      scale;
...
referenceCanvas.addEventListener(
  ...
  event => {
    ...
    referenceCanvas.setPointerCapture(
      event.pointerId
    );
...
function drawChart(
  ...
  if (!rows.length) {
    ...
    g.fillText(
      'Track a target to build your session history.',
      w / 2,
      h / 2
    );
...
// ============================================================
// APPEARANCE / NCC CHART
// ============================================================

function drawAppearanceChart(
  id,
  color
) {
  ...
  // NCC naturally ranges from -1 to +1.
  const minValue =
    -1.0;
  ...
  if (!rows.length) {
    ...
    g.fillText(
      'Track a target to build appearance history.',
      w / 2,
      h / 2
    );
...
function route() {
  ...
  const titles = {
    ...
    dsp: [
      'DSP Lab',
      '02 / INSPECT + UNDERSTAND',
      'Look inside the signal behind the movement.'
    ],
...
function uploadVideo(file) {
  ...
  if (
    ...
    notice(
      'Please choose a video smaller than 512 MB.',
      true
    );
...
$('target-image-file').onchange =
  async event => {
    ...
    if (
      ...
      notice(
        'Please choose a target image smaller than 20 MB.',
        true
      );
...
$('reference-roi').onclick =
  async () => {
    ...
    if (
      ...
      notice(
        'Open the target image again so its native pixels can be selected.',
        true
      );
    ...
    notice(
      'Drag a tight rectangle around the target in the uploaded image. The crop is not resized.'
    );
...
$('roi').onclick =
  async () => {
    ...
    notice(
      'Drag around a textured target in the video. This switches to normal video-ROI mode.'
    );
...
$('export').onclick =
  async () => {
    ...
    try {
      ...
      notice(
        'Session CSV exported. Playback is paused.'
      );
...
document.addEventListener(
  ...
  () => {
    ...
    if (
      ...
      notice(
        'Playback paused while the tab is hidden.'
      );
    }
  }
);
  ...
  .then(
    s => {
      ...
      if (
        ...
        notice(
          'Workspace restored. Re-open the target image only if you want to change its ROI.'
        );
        ...
        notice(
          'Workspace restored. Press Play to continue.'
        );
      }
    }
  )
